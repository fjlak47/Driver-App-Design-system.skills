...
- 设计规范基于 `750px` 宽度画布制定，应用时也应保持 `750px` 宽度画布（需自适应大小）
- 所有页面宽度统一为 `750px`，使用弹性布局实现自适应
...
- 所有内容卡片统一使用以下阴影和圆角：
  ...
  - 背景色：`#FFFFFF`
  - 内边距：`24px`
  - 卡片间距：`16px`（margin-bottom）
- 字号使用规范值，确保在750px宽度画布下显示清晰，避免使用过小的字号
- 适配技巧：在body样式中设置`width: 100%; max-width: 750px; margin: 0 auto;`，确保页面在750px宽度下居中显示
- 适配技巧：移除所有子容器中的`max-width`和`margin: 0 auto;`属性，因为body已经控制了整体宽度
- 适配技巧：将媒体查询断点设置为`max-width: 750px`，确保在750px宽度时使用正常的样式，而不是小屏幕的样式
